...
mod tests {
    ...
    use petgraph_core::{
        edge::Undirected,
        index::IndexType,
        visit::{Dfs, EdgeRef, IntoEdgeReferences, IntoNodeReferences, VisitMap},
    };

    use super::Csr;
    use crate::NodeIndex;

    fn n<Ix>(x: Ix) -> NodeIndex<Ix>
    where
        Ix: IndexType,
    {
        NodeIndex::new(x)
    ...
    fn csr1() {
        ...
        m.add_edge(1, 1, ());
        println!("{:?}", m);
        assert_eq!(&m.column, &[n(0), n(2), n(0), n(1), n(2), n(2)]);
        assert_eq!(&m.row, &[0, 2, 5, 6]);

        let added = m.add_edge(NodeIndex::new(1), NodeIndex::new(2), ());
        assert!(!added);
        assert_eq!(&m.column, &[n(0), n(2), n(0), n(1), n(2), n(2)]);
        assert_eq!(&m.row, &[0, 2, 5, 6]);

        assert_eq!(m.neighbors_slice(NodeIndex::new(1)), &[n(0), n(1), n(2)]);
    ...
    fn csr_from_error_1() {
        ...
        let m: Csr = Csr::from_sorted_edges(&[(n(0), n(1)), (n(1), n(0)), (n(0), n(2))]).unwrap();
        println!("{:?}", m);
    }
    ...
    fn csr_from_error_2() {
        ...
        let m: Csr =
            Csr::from_sorted_edges(&[(n(0), n(1)), (n(1), n(0)), (n(1), n(2)), (n(1), n(1))])
                .unwrap();
        println!("{:?}", m);
    }
    ...
    fn test_find_neg_cycle4() {
        let m: Csr<(), _> = Csr::from_sorted_edges(&[(n(0), n(0), -1.)]).unwrap();
        let result = find_negative_cycle(&m, 0);
