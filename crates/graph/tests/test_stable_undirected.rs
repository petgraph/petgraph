...
fn edges_multi_connecting() {
    ...
    assert_eq!(graph.edges_connecting(b, a).count(), 2);
...
fn neighbours_directed() {
    ...
    assert_eq!(graph.neighbors_directed(a, Direction::Incoming).count(), 1);

    assert_eq!(graph.neighbors_directed(b, Direction::Outgoing).count(), 1);
