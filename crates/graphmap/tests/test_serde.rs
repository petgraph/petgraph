...
fn example() -> MapStorage<i32, u32, Directed> {
    let [a, b, c, d, e, f] = [0, 1, 2, 3, 4, 5];

    let mut graph = MapStorage::from_edges([
        (a, b, 7),
        (c, a, 9),
        (a, d, 14),
        (b, c, 10),
...
fn assert_graph_eq<N, E, Ty>(graph1: &MapStorage<N, E, Ty>, graph2: &MapStorage<N, E, Ty>)
where
...
fn deserialize() {
    ...
    let deserialized: MapStorage<i32, u32, Directed> = MapStorage::deserialize(serialized).unwrap();
...
fn example_struct() -> MapStorage<Point, i32, Directed> {
    let a = Point { x: 0, y: 0 };
    let b = Point { x: 1, y: 1 };
    let c = Point { x: 2, y: 2 };
...
fn deserialize_struct() {
    ...
    let serialized = serde_value::to_value(&graph).unwrap();
    let deserialized: MapStorage<Point, i32, Directed> =
        MapStorage::deserialize(serialized).unwrap();
