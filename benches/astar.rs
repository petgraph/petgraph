...
#[bench]
fn astar_maze2d_line_bench(bench: &mut Bencher) {
    // Same maze as ./dijkstra.rs
    const N: usize = 500;
    const M: usize = 5;
    let g = common::build_2d_maze(N, M);
...
#[bench]
fn astar_maze2d_bench(bench: &mut Bencher) {
    // Same maze as ./dijkstra.rs
    const N: usize = 500;
    let g = common::build_2d_maze(N, N);
