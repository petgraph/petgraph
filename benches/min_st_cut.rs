...
#[bench]
fn min_st_cut_bench(bench: &mut Bencher) {
...
#[bench]
fn min_st_cut_bench_many_edges(bench: &mut Bencher) {
...
#[bench]
fn min_st_cut_bench_wide(bench: &mut Bencher) {
    let node_count = 1000;
    let mut g: Graph<usize, usize> = Graph::new();
    ...
    bench.iter(|| {
        let _result = min_st_cut(&g, source, sink);
    });
}

#[bench]
fn min_st_cut_bench_dense_middle(bench: &mut Bencher) {
    let node_count = 500;
    let mut g: Graph<usize, usize> = Graph::new();
    let source = g.add_node(0);
    let sink = g.add_node(1);
...
#[bench]
fn min_st_cut_bench_dense_middle_varying_weights(bench: &mut Bencher) {
    let node_count = 500;
    let mut g: Graph<usize, usize> = Graph::new();
    let source = g.add_node(0);
    let sink = g.add_node(1);
