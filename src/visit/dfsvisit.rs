    ...
    'prune: loop {
        ...
        for v in graph.neighbors(u) {
            if !discovered.is_visited(&v) {
                try_control!(visitor(DfsEvent::TreeEdge(u, v)), break 'prune);
                try_control!(dfs_visitor(graph, v, visitor, discovered, finished, time),
                             unreachable!());
            } else if !finished.is_visited(&v) {
                try_control!(visitor(DfsEvent::BackEdge(u, v)), break 'prune);
            } else {
                try_control!(visitor(DfsEvent::CrossForwardEdge(u, v)), break 'prune);
            }
        }
